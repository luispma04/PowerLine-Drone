...
import com.dji.sdk.sample.demo.missionoperator.ui.MissionControlView;
...
public class MainActivity extends AppCompatActivity implements MissionControlView.FilePickerCallback {
    ...
    private MissionControlView missionControlView;

    // Flag to easily determine if we're running in simulator mode
    private boolean isSimulatorMode = false; // Set to true by default for testing
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Create the Mission Control View (updated from StructureInspectionMissionView)
        missionControlView = new MissionControlView(this, isSimulatorMode);
        ...
        // Add the mission control view to the content frame
        contentFrameLayout.addView(missionControlView);

        // Push the view to the stack with the appropriate title
        stack.push(new ViewWrapper(missionControlView, R.string.component_listview_structure_inspection_mission));
        ...
        DJISDKManager.getInstance().registerApp(this.getApplicationContext(), new DJISDKManager.SDKManagerCallback() {
            ...
            public void onRegister(DJIError djiError) {
                ...
                if (djiError == DJISDKError.REGISTRATION_SUCCESS) {
                    ...
                    // Update connection status in MissionControlView
                    if (missionControlView != null) {
                        runOnUiThread(() -> {
                            missionControlView.onStatusUpdate("SDK registered successfully");
                        });
                    }
                } else {
                    ...
                    if (missionControlView != null) {
                        runOnUiThread(() -> {
                            missionControlView.onStatusUpdate("SDK registration failed: " + djiError.getDescription());
                        });
                    }
                }
            }
            ...
            public void onProductDisconnect() {
                ...
                if (missionControlView != null) {
                    runOnUiThread(() -> {
                        missionControlView.onStatusUpdate("Aircraft disconnected");
                    });
                }
            }
            ...
            public void onProductConnect(BaseProduct baseProduct) {
                ...
                if (missionControlView != null) {
                    runOnUiThread(() -> {
                        missionControlView.onProductConnected();
                        missionControlView.onStatusUpdate("Aircraft connected");
                    });
                }
            }
            ...
            public void onProductChanged(BaseProduct baseProduct) {
                ...
                if (missionControlView != null) {
                    runOnUiThread(() -> {
                        missionControlView.onStatusUpdate("Aircraft changed");
                    });
                }
            }
                ...
                if (newComponent != null) {
                    newComponent.setComponentListener(new BaseComponent.ComponentListener() {
                        ...
                        public void onConnectivityChange(boolean isConnected) {
                            ...
                            if (missionControlView != null) {
                                runOnUiThread(() -> {
                                    missionControlView.onStatusUpdate(componentName + (isConnected ? " connected" : " disconnected"));
                                });
                            }
                        }
                    });
            ...
            public void onInitProcess(DJISDKInitEvent djisdkInitEvent, int i) {
                // Update initialization progress in MissionControlView
                if (missionControlView != null) {
                    runOnUiThread(() -> {
                        missionControlView.onStatusUpdate("SDK Initialization: " + djisdkInitEvent.toString() + " " + i);
                    });
                }
            }
            ...
            public void onDatabaseDownloadProgress(long current, long total) {
                // Update database download progress in MissionControlView
                if (missionControlView != null) {
                    runOnUiThread(() -> {
                        missionControlView.onStatusUpdate("Database Download Progress: " + current + "/" + total);
                    });
                }
            }
        });
    ...
    public void onBackPressed() {
        // If this is the only view (our Mission Control view), just exit the app
        if (stack.size() <= 1) {
            finish();
        } else {
    ...
    public void onConfigurationChanged(@NonNull Configuration newConfig) {
        ...
        // Pass the configuration change to the current view if it's a MissionControlView
        if (stack.size() > 0 && stack.peek().getView() instanceof MissionControlView) {
            ((MissionControlView) stack.peek().getView()).onConfigurationChanged(newConfig);
        }
    }
    ...
    private void refreshTitle() {
        // If this is the only view in stack, it's our Mission Control view
        if (stack.size() == 1) {
    ...
    private void refreshOptionsMenu() {
        ...
        // Show hint if the current view is a PresentableView (Mission Control is one)
    ...
    // Implementation of FilePickerCallback interface
    @Override
    public void openFilePicker(int requestCode) {
        ...
        try {
            ...
            startActivityForResult(Intent.createChooser(intent, "Select CSV file"), requestCode);
        } catch (Exception e) {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK && data != null && data.getData() != null) {
            if (requestCode == 1001 || requestCode == 1002) {
                // Find the current mission control view
                if (stack.size() > 0) {
                    View currentView = stack.peek().getView();
                    if (currentView instanceof MissionControlView) {
                        ((MissionControlView) currentView).onFileSelected(requestCode, data.getData());
                    }
                }
            }
        }
    ...
    public void onConnectivityChange(ConnectivityChangeEvent event) {
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                refreshTitle();
                // Update the connection status in the MissionControlView
                if (missionControlView != null) {
                    // Connection status is handled automatically by the new architecture
                }
            }
        });
    }

    // Event classes - keep these or create them in a separate events package
